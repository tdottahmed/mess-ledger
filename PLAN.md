...
Accounts to create first: [ACCOUNTS.md](./ACCOUNTS.md)
...
2. **Set PIN** — first login on a device sets a 4-digit PIN, stored as
   `HMAC-SHA-256(pin, per-member salt)` under a pepper held in a Worker secret, never in D1.
   Rate-limited to 5 attempts, then a 15-minute lockout keyed on the phone number.

   > **Not bcrypt or scrypt.** Both land in the 10–20 ms range, and the free plan gives the Worker
   > 10 ms of CPU per request — login would simply fail. `ctx.waitUntil()` is no escape: its 30 s is
   > wall-clock, the CPU budget is unchanged. Nor is the loss real. A 4-digit PIN is a
   > 10,000-candidate space, so no KDF meaningfully slows anyone holding the database. The security
   > here comes from the pepper living outside D1, the 5-attempt lockout, and an unknown phone
   > number being unable to get in at all. If a slow KDF is ever wanted, run argon2id in the
   > browser — no CPU limit there — and keep the server's half cheap.
...
2. **Outbox, not retries** — every mutation is appended to an outbox table with its payload, then
   applied to Dexie. A single flush loop drains it in order, idempotent by mutation id. Flush at
   most **40 mutations per request** — D1 allows 50 queries per Worker invocation on the free plan,
   and a backlog after a week offline would blow straight through it.
...
- [ ] Preview image for shared links, rendered on the closing device at month close and uploaded to R2 (never server-side — 10 ms CPU cannot rasterise)
...
|---|---|---|
| Workers | 100,000 requests/day; **10 ms CPU per request and per cron** | ~200/day across three phones |
| Static assets | Free and unlimited, no Worker invocation | The whole app shell |
| D1 | 500 MB per database, 5 GB per account, 5M rows read/day | Under 10 MB after years |
| D1 queries | **50 per Worker invocation** | 8 for a sync pull; outbox flush capped at 40 |
...
The 10 ms CPU ceiling is the only tight one, and it settles two design questions rather than one: the
PIN is HMAC'd rather than run through bcrypt (§5), and the shared link's preview image is **rendered
on the closing device** and uploaded to R2 rather than generated by the Worker at all.
